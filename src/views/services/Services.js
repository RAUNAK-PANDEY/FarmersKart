  ...
  const onExportData = async () => {
    ...
    const filteredData = state.orders
      ...
      .map((order) => ({
        ...
        societyWeight:order.society.map((sub) => {
          return sub.weight+sub.unit;
        }).toString(),
        societyPrice:order.society.map((sub) => {
          return sub.discountedPrice;
        }).toString(),
        shopWeight:order.shop.map((sub) => {
          return sub.weight+sub.unit;
        }).toString(),
        shopPrice:order.shop.map((sub) => {
          return sub.discountedPrice;
        }).toString(),
        hotelWeight:order.hotel.map((sub) => {
          return sub.weight+sub.unit;
        }).toString(),
        hotelPrice:order.hotel.map((sub) => {
          return sub.discountedPrice;
        }).toString(),
