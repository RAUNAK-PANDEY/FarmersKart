...
const CreditPoints = (props) => {
 ...
 const credit = async (e) => {
    ...
    if (form.checkValidity() === false) {
        ...
        e.preventDefault()
        setValidated(true)
        await firebase.firestore().collection("users").doc(props.location.state.id).collection("wallet").add({
        ...
        await firebase.firestore().collection("users").doc(props.location.state.id).update({
            walletAmount:firebase.firestore.FieldValue.increment(formData.values.amount.valueOf())
        });
