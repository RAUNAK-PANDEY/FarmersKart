...
const EditUser = (props) => {
  ...
  const initialFormData = {
    ...
    gst:props.location.state.gstNO,
    societyName:props.location.state.societyName,
    wing: props.location.state.wing,
    fno:props.location.state.flatNo,
  ...
  const updatedStatus = async (s, i,a) => {
    ...
    formData.values.address = a.toString();
    //   cat.filter((x) => x.id === status.id)
    //   .map((sub) => {
    //     console.log(sub);
    ...
    //   })
    // )
    console.log(formData.values.address);
  };
...
//   }
  ...
  const edit = async (e) => {
    e.preventDefault();
    await firebase.firestore().collection("users").doc(props.location.state.id).update({
    name:formData.values.name ,
    mobile:formData.values.mobile,
    email:formData.values.email,
    // gstNo:formData.values.gst,
    societyName:status.name,
    wing:formData.values.wing,
    flatNo:formData.values.fno,
      ...
      <CCol>
        ...
        <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >
          ...
          <CForm className="row g-3">
                        ...
                        {cat &&
                          cat.map((cat, index) => {
                            return (
                              <CDropdownItem
                              required
                                onClick={() => updatedStatus(cat.centerName, cat.docId,cat.address)}
                              >
                                {cat.centerName}
                              </CDropdownItem>
                            );
                ...
                <CCol md={6}>
                    <CLabel htmlFor="inputPassword4">Wing</CLabel>
                    <CInput
                      required 
                      type="text"
                      placeholder="Wing"
                      name="wing"
                      value={formData.values.wing}
                      onChange={(e) => {
                        formData.handleChange(e);
                        // setFormData({
                        //   ...formData.values,
