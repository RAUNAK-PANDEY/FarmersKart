...
const Instagram = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("generalData").doc("banners").collection("instagram").get();
  ...
  const toggle = async(rowId,colId) => {
   if(colId===true){
    await firebase.firestore().collection("generalData").doc("banners").collection("instagram").doc(rowId).update({
      isActive:false,
    })
    getVideos();
   }else{
      ...
      buttons: [
        {
          label: "Yes",
          onClick: async() => {
            await firebase.firestore().collection("generalData").doc("banners").collection("instagram").doc(rowId).delete();
            alert("Image Deleted");
            history.push('/');
  ...
  const edit = async(rowId) => {
    history.push(
      {
      pathname: '/banner/edit-instagram',
      state: rowId
      }
    )
  };
  ...
  return (
    <CRow>
      ...
      <CCol>
        <CCard>
        <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Instagram Image List</CCardHeader>
          <CCardBody style={{textAlign:"center"}}>
            <CDataTable style={{border:"5px solid black",textAlign: "center"}}
              ...
              fields={[
                ...
                { key: "sequence", label: "Image Sequence",filter: true },
                { key: "image", label:"Instagram Image" }, 
