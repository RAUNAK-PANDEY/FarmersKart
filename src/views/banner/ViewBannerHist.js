...
const ViewBannerHist = (props) => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("generalData").doc("banners").collection("banners").doc(props.location.state.id).collection("clicks").where("date", ">=", order).where("date", "<=", porder).get();
    ...
    <CRow>
      ...
      <CCol>
        ...
        <CCardHeader
          ...
          >
              <CCol sm="3">
                    <div className="font-xl">Banner History Report</div>
                </CCol>
                <CCol sm="1">
