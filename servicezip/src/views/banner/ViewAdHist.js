...
const ViewAdHist = (props) => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("generalData").doc("banners").collection("advertisement").doc(props.location.state.id).collection("clicks").where("date", ">=", order).where("date", "<=", porder).get();
    ...
    <CRow>
      ...
      <CCol>
        ...
        <CCardHeader
          ...
          >
              <CCol sm="3">
                    <div className="font-xl">Ad History Report</div>
                </CCol>
                <CCol sm="1">
