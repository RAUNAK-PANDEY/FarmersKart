...
const HSN = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const response=await firebase.firestore().collection("generalData").doc("hsn").get();
    response.data().hsn.map(sub1 =>{
...
//   }
  ...
  return (
    <CRow>
      ...
      <CCol>
        <CCard>
        <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >HSN Number List</CCardHeader>
          <CCardBody>
            <CDataTable style={{border:"1px solid #ebedf0"}}
              ...
              fields={[
                ...
                { key: "cat", label: "HSN Number",filter: true },
                // { key: "status" },
