...
//   console.log(state.users);
  const getPostorder = async () => {
    setLoading(true);
    const users = await firebase.firestore().collection("orders").where("userType","==","Shop").where("isCancelled","==",false).where("isCancelled","==",false).where('orderStatus', '==', 'processed').get();
  ...
  const getLorder = async () => {
    setLoading(true);
    const users = await firebase.firestore().collection("orders").where("userType","==","Shop").where("isCancelled","==",false).where("isCancelled","==",false).where('orderStatus', '==', 'picked').get();
  ...
  const getDeliverorder = async () => {
    setLoading(true);
    const users = await firebase.firestore().collection("orders").where("userType","==","Shop").where("isCancelled","==",false).where("isCancelled","==",false).where('orderStatus', '==', 'delivered').get();
