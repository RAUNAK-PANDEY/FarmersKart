...
const EmployeeHist = (props) => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("employee").doc(props.location.index).collection("salary").get();
...
//   }
  ...
  return (
    <CRow>
      ...
      <CCol>
        <CCard>
        <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}}>Employee Transaction History</CCardHeader>
          <CCardBody>
            <CRow>
                <CDataTable
                    loading={loading}
