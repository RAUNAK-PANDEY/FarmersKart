...
//   const [lastOrder, setLastOrder] = useState("");
  ...
  const updatedStatus = async (cid,cname,cnumber,cemail) => {
    setStatus({id:cid,name:cname,mobile:cnumber,email:cemail})
    console.log(status);
  };
                                  ...
                                  <CDropdownMenu style={{ width: "100%"}}>
                                    ...
                                    <CDropdownItem divider />
                                    {
                                        gdata && gdata.map((cat,index)=>{
                                            return(
                                            <CDropdownItem onClick={() => updatedStatus(cat.id,cat.customerName,cat.customerNumber,cat.customerEmail)}><div><div>{cat.customerName}</div><div>{cat.mobile}</div></div></CDropdownItem>
                                            )
                                        })
                                    }
                                  </CDropdownMenu>
